...
public class XPathValidity {
	...
	public void step(String token) {
		...
		if (token.contains("[")) {
			...
			// if indxOfAxis > indxOfBrac test appears before axis or end of
			// current node
			if (indxOfAxis > indxOfBrac || indxOfAxis == -1) {
				...
				if (bracIndx.isEmpty() && i >= token.length()) {
					...
					if (!isGlobal)
						state.pop();
				} else if (!bracIndx.isEmpty() || bracIndx.isEmpty()
						&& !token.matches("\\s*/\\s*[.*?]*")) {
					valid = false;
				}
			} else {
	...
	public void test(String subToken) {
		if (subToken
				.matches("\\s*contains\\s*\\(\\s*text\\s*\\(\\)\\s*,\\s*\\\".*?\\\"\\s*\\)\\s*")) {
			System.out.println("Found contains in test");
			return;
		} else if (subToken
				.matches("\\s*text\\s*\\(\\s*\\)\\s*=\\s*\\\"\\s*.*?\\s*\\\"\\s*")) {
			System.out.println("Found text in test");
			return;
		} else if (subToken
				.matches("\\s*@\\s*[a-zA-Z0-9]*\\s*=\\s*\\\"\\s*[a-zA-Z0-9]*\\s*\\\"\\s*")) {
			...
			// send it to step()
			// System.out.println("Subtoken:" + subToken + " Match: " +
			// subToken.matches("[a-zA-Z0-9\\-]*") + "   token: " + token);
			if (subToken.matches("^(.+)/([^/]+)$")) {
				step(subToken);
			} else {
				valid = false;
