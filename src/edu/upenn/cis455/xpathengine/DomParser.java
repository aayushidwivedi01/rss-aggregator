					...
					for (int f = 0; f < filters.size(); f++) {
						...
						if (filter
							...
							if (elementText.contains(queryText)) {
								...
								.matches("@\\s*[a-zA-Z0-9]*\\s*=\\s*\\\"\\s*[a-zA-Z0-9]*\\s*\\\"\\s*")) {
							...
							String attrName = filter
									.substring(indxA + 1, indxE).trim();
							Node nodeAttr = attMap.getNamedItem(attrName);
							if (nodeAttr != null
									&& attrName.equals(nodeAttr.getNodeName())) {
								int indxO = filter.indexOf("\"");
								int indxC = filter.lastIndexOf("\"");
								String attrValue = filter.substring(indxO + 1,
										indxC);
							...
							if (filterParser.parse(xpathValid.allNodes,
									currrNode)) {
								foundAll = true;
								continue;
							} else {
				...
				for (int j = 0; j < allChildren.getLength(); j++) {
					if (pn.nodeName.equals(allChildren.item(j).getNodeName())) {
						...
						if (pn.filters.size() == 0) {
							...
							System.out.println("New curr node:"
									+ currrNode.getNodeName());
							allChildren = currrNode.getChildNodes();
							found = true;
							break;
						} else {
							...
							for (int f = 0; f < filters.size(); f++) {
								...
								if (filter
										.matches("contains\\s*\\(\\s*text\\s*\\(\\)\\s*,\\s*\\\".*?\\\"\\s*\\)\\s*")) {
									String elementText = allChildren.item(j)
											.getTextContent();
									int indxO = filter.indexOf("\"");
									int indxC = filter.lastIndexOf("\"");
									String queryText = filter.substring(
											indxO + 1, indxC);
									if (elementText.contains(queryText)) {
										found = true;
										...
										if (f == filters.size() - 1) {
											currrNode = allChildren.item(j);
											allChildren = currrNode
													.getChildNodes();
											System.out
													.println("Found all filters");

											System.out
													.println("Final curr node:"
															+ currrNode);
											break;
										}
									}
										...
										.matches("@\\s*[a-zA-Z0-9]*\\s*=\\s*\\\"\\s*[a-zA-Z0-9]*\\s*\\\"\\s*")) {
									// check if current node has this
									// attributename
									NamedNodeMap attMap = allChildren.item(j)
											.getAttributes();
									int indxA = filter.indexOf("@");
									int indxE = filter.indexOf("=");
									String attrName = filter.substring(
											indxA + 1, indxE).trim();
									Node nodeAttr = attMap
											.getNamedItem(attrName);
									if (nodeAttr != null
											&& attrName.equals(nodeAttr
													.getNodeName())) {
										int indxO = filter.indexOf("\"");
										int indxC = filter.lastIndexOf("\"");
										String attrValue = filter.substring(
												indxO + 1, indxC);
										if (attrValue.equals(nodeAttr
												.getNodeValue()))
											found = true;

										// check if all filters have been
										// checked
										if (f == filters.size() - 1) {
											currrNode = allChildren.item(j);
											allChildren = currrNode
									...
									filter = allChildren.item(j).getNodeName()
											+ "/" + filter;
